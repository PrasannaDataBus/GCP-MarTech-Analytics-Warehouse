...
from meta_ads_campaign_dim import main as meta_ads_main
...
with DAG(
    dag_id="meta_ads_campaign_dim_raw_dag",
    default_args=default_args,
    description="Incrementally extract and load Meta Ads Campaign dimensions data into BigQuery",
    schedule_interval="0 8 * * *",  # Runs daily at 8 AM
    start_date=datetime(2025, 11, 1),
    catchup=False,
    tags=["meta_ads", "bigquery", "dimension", "prod"],
) as dag:

    run_incremental_extraction = PythonOperator(
        task_id="extract_and_load_meta_ads_campaign_dim_raw_data",
