...
# Reliability Logic: Implemented 30-day Date Chunking to prevent API Timeouts during backfills ('hourly_stats_aggregated_by_advertiser_time_zone' will have huge volume of records)
...
META_HOURLY_TABLE_NAME = os.getenv("META_HOURLY_TABLE_NAME")

# --- AUTHENTICATION ---
bq_client = bigquery.Client()

# --- FIELDS TO EXTRACT ---
# Note: 'hourly_stats_aggregated_by_advertiser_time_zone' are NOT in fields, they come from 'breakdowns' param
FIELDS = [
    'date_start',
    'date_stop',
    'account_id',
...
def extract_hourly_data(account_id: str, account_name: str, start_date: str, end_date: str, currency: str):
    """Extracts Hourly data for a specific Meta Ad Account using date chunking."""
    ...
    for chunk_start, chunk_end in get_date_chunks(start_date, end_date, chunk_size = 30):
        ...
        params = {
            ...
            'time_increment': 1,
            'level': 'ad',
            'breakdowns': ['hourly_stats_aggregated_by_advertiser_time_zone'], # <--- THE KEY DIFFERENCE
            'limit': 1000,
        ...
        try:
            ...
            for item in result_cursor:
                ...
                raw_hour = item.get('hourly_stats_aggregated_by_advertiser_time_zone', '00:00:00 - 00:59:59')
                hour_int = int(raw_hour.split(':')[0])
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{META_HOURLY_TABLE_NAME}"
    try:
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_id: str, bq_client):
    ...
    job_config = bigquery.LoadJobConfig(
        ...
        clustering_fields = ["account_id", "campaign_id", "hour_of_day"]
    )
...
def main():
    ...
    print("--- Starting Meta Ads 'hourly_stats_aggregated_by_advertiser_time_zone' Extraction ---")
    ...
    for acc in accounts:
        ...
        try:
            df = extract_hourly_data(acc_id, acc_name, start_date, end_date, currency)
            if not df.empty:
