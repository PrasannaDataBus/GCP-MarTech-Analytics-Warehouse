...
PMAX_GEO_TABLE_NAME = os.getenv("PMAX_GEO_TABLE_NAME")
...
def extract_pmax_geo_data(customer_id: str, start_date: str, end_date: str, ads_client):
    """Extracts PMAX GEO performance data from Google Ads for a specific account and date range."""
    service = ads_client.get_service("GoogleAdsService")
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{PMAX_GEO_TABLE_NAME}"
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_name: str, account_id: str, bq_client):
    ...
    job_config = bigquery.LoadJobConfig(write_disposition="WRITE_APPEND",
                                        # Define the full schema. This is essential for creating
                                        # the table correctly the first time with partitioning/clustering.
                                        schema = [
                                            ...
                                            bigquery.SchemaField("geo_criterion_id", "STRING"),
    ...
    job_config.time_partitioning = bigquery.TimePartitioning(
        ...
        field = "date",  # The column to partition by
    )
    ...
    job.result()
    print(f"Loaded {len(df)} rows for {account_name} ({account_id}) into {RAW_DATASET_NAME}.{PMAX_GEO_TABLE_NAME}")
...
def main():
    ...
    for account in child_accounts:
        ...
        try:
            # Step 1: Extract
            df = extract_pmax_geo_data(customer_id, start_date, end_date, ads_client)
            if df.empty:
