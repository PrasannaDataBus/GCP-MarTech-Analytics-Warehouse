...
TARGETED_GEO_CONV_TABLE_NAME = os.getenv("TARGETED_GEO_CONV_TABLE_NAME")

# --- GAQL Query (NO PRE-CLICK METRICS allowed here!)---
QUERY_TEMPLATE = """
SELECT
  segments.date,
  customer.id,
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{TARGETED_GEO_CONV_TABLE_NAME}"
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_name: str, account_id: str, bq_client):
    ...
    job.result()
    print(f"Loaded {len(df)} rows for {account_name} ({account_id}) into {RAW_DATASET_NAME}.{TARGETED_GEO_CONV_TABLE_NAME}")
...
def main():
    ...
    for account in child_accounts:
        ...
        try:
            # Step 1: Extract
            df = extract_targeted_geo_conversions(customer_id, start_date, end_date, ads_client)
            if df.empty:
...
#         customer_id = str(account["id"])
#         account_name = account["name"]
#         print(f"\nExtracting USER LOCATION CONVERSIONS for account: {account_name} ({customer_id})")
#
...
#
#             print(f"Extracting USER LOCATION CONVERSIONS {start_date} → {end_date}")
#             try:
#                 df = extract_targeted_geo_conversions(customer_id, start_date, end_date, ads_client)
#
#                 if df.empty:
#                     print(f"No USER LOCATION CONVERSIONS data for {yr} in {account_name}")
#                     continue
#
#                 print(f"Extracted USER LOCATION CONVERSIONS {len(df)} rows for {yr} ({account_name})")
#                 load_to_bigquery(df, bq_client)
#
#             except Exception as e:
#                 print(f"Failed USER LOCATION CONVERSIONS for {account_name} ({customer_id}): {e}")
#
#
# if __name__ == "__main__":
#     main()
