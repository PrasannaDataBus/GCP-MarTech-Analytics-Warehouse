...
# Reliability Logic: Implemented 30-day Date Chunking to prevent API Timeouts during backfills (country will have huge volume of records)
...
META_COUNTRY_TABLE_NAME = os.getenv("META_COUNTRY_TABLE_NAME")

# --- AUTHENTICATION ---
bq_client = bigquery.Client()

# --- FIELDS TO EXTRACT ---
# Note: 'country' are NOT in fields, they come from 'breakdowns' param
FIELDS = [
    'date_start',
    'date_stop',
    'account_id',
...
def extract_country_data(account_id: str, account_name: str, start_date: str, end_date: str, currency: str):
    """Extracts Country data for a specific Meta Ad Account using date chunking."""
    ...
    for chunk_start, chunk_end in get_date_chunks(start_date, end_date, chunk_size = 30):
        ...
        params = {
            ...
            'time_increment': 1,
            'level': 'ad',
            'breakdowns': ['country'], # <--- THE KEY DIFFERENCE
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{META_COUNTRY_TABLE_NAME}"
    try:
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_id: str, bq_client):
    ...
    job_config = bigquery.LoadJobConfig(
        ...
        clustering_fields = ["account_id", "campaign_id", "country"]
    )
...
def main():
    ...
    print("--- Starting Meta Ads Country Extraction ---")
    ...
    for acc in accounts:
        ...
        try:
            df = extract_country_data(acc_id, acc_name, start_date, end_date, currency)
            if not df.empty:
