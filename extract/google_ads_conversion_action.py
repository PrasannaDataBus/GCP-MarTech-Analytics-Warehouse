...
CONVERSION_ACTION_TABLE_NAME = os.getenv("CONVERSION_ACTION_TABLE_NAME")

# --- GAQL Query for CONVERSION ACTION---
QUERY_TEMPLATE = """
SELECT
  segments.date,
  customer.id,
...
def extract_conversion_data(customer_id: str, start_date: str, end_date: str, ads_client):
    """Extracts Conversion Action performance data from Google Ads for a specific account and date range."""
    service = ads_client.get_service("GoogleAdsService")
    ...
    for batch in response:
        for row in batch.results:
            rows.append({
                ...
                "conversion_action_id": str(row.segments.conversion_action).split('/')[-1], # Parse ID from resource name
                "conversion_action_name": row.segments.conversion_action_name,
                "conversion_category": row.segments.conversion_action_category.name, # e.g., PURCHASE, LEAD
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{CONVERSION_ACTION_TABLE_NAME}"
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_name: str, account_id: str, bq_client):
    ...
    job_config = bigquery.LoadJobConfig(write_disposition="WRITE_APPEND",
                                        schema = [
                                            ...
                                            bigquery.SchemaField("conversion_action_id", "STRING"),
                                            bigquery.SchemaField("conversion_action_name", "STRING"),
                                            bigquery.SchemaField("conversion_category", "STRING"),
    ...
    job.result()
    print(f"Loaded {len(df)} rows for {account_name} ({account_id}) into {RAW_DATASET_NAME}.{CONVERSION_ACTION_TABLE_NAME}")
...
def main():
    ...
    lookback_days = 180  # configurable window for late updates
    if last_loaded_date:
    ...
    for account in child_accounts:
        ...
        try:
            # Step 1: Extract
            df = extract_conversion_data(customer_id, start_date, end_date, ads_client)
            if df.empty:
...
#         customer_id = str(account["id"])
#         account_name = account["name"]
#         print(f"\nExtracting CONVERSION ACTION DATA for account: {account_name} ({customer_id})")
#
...
#
#             print(f"Extracting CONVERSION ACTION {start_date} → {end_date}")
#             try:
#                 df = extract_conversion_data(customer_id, start_date, end_date, ads_client)
#
#                 if df.empty:
#                     print(f"No CONVERSION ACTION data for {yr} in {account_name}")
#                     continue
#
#                 print(f"Extracted CONVERSION ACTION {len(df)} rows for {yr} ({account_name})")
#                 load_to_bigquery(df, bq_client)
#
#             except Exception as e:
#                 print(f"Failed CONVERSION ACTION DATA for {account_name} ({customer_id}): {e}")
#
#
# if __name__ == "__main__":
#     main()
