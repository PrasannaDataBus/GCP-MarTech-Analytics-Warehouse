...
BUDGET_PACING_TABLE_NAME = os.getenv("BUDGET_PACING_TABLE_NAME")

# --- GAQL Query for BUDGET PACING---
QUERY_TEMPLATE = """
SELECT
  segments.date,
  customer.id,
  ...
  campaign_budget.amount_micros,     -- The Daily Target Budget
...
def extract_budget_data(customer_id: str, start_date: str, end_date: str, ads_client):
    """Extracts Campaign Budget and Pacing data from Google Ads for a specific account and date range."""
    service = ads_client.get_service("GoogleAdsService")
    ...
    if not df.empty:
        ...
        df = df.astype({
            ...
            "total_budget_micros": "int64",
            "cost_micros": "int64",
            "search_budget_lost_is": "float64",
            "content_budget_lost_is": "float64",
    })
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_name: str, account_id: str, bq_client):
    ...
    job_config = bigquery.LoadJobConfig(write_disposition="WRITE_APPEND",
                                        schema = [
                                            ...
                                            bigquery.SchemaField("daily_budget_micros", "INTEGER"),
    ...
    job.result()
    print(f"Loaded {len(df)} rows for {account_name} ({account_id}) into {RAW_DATASET_NAME}.{BUDGET_PACING_TABLE_NAME}")
...
def main():
    ...
    for account in child_accounts:
        ...
        try:
            # Step 1: Extract
            df = extract_budget_data(customer_id, start_date, end_date, ads_client)
            if df.empty:
...
#
#             print(f"Extracting BUDGET PACING DATA {start_date} → {end_date}")
#             try:
#                 df = extract_budget_data(customer_id, start_date, end_date, ads_client)
#
#                 if df.empty:
#                     print(f"No BUDGET PACING data for {yr} in {account_name}")
#                     continue
#
#                 print(f"Extracted BUDGET PACING DATA {len(df)} rows for {yr} ({account_name})")
#                 load_to_bigquery(df, bq_client)
#
#             except Exception as e:
#                 print(f"Failed BUDGET PACING DATA for {account_name} ({customer_id}): {e}")
#
#
# if __name__ == "__main__":
#     main()
