...
META_CONV_ACTION_TABLE_NAME = os.getenv("META_CONV_ACTION_TABLE_NAME")
...
FIELDS = [
    ...
    'ad_id',
    'ad_name',
    'actions',  # List of Conversion Counts
    'action_values'  # List of Conversion Revenues
]
...
def extract_conversion_action_data(account_id: str, account_name: str, start_date: str, end_date: str, currency: str):
    """Extracts Conversion Actions using ASYNC Job and Explodes JSON Lists using date chunking."""
    ...
    for chunk_start, chunk_end in get_date_chunks(start_date, end_date, chunk_size = 30):
        ...
        try:
            ...
            for item in result_cursor:
                # --- ROW EXPLOSION LOGIC START ---

                # 1. Map Revenue: { 'purchase': 100.00, 'lead': 0.00 }
                revenue_map = {}
                if 'action_values' in item:
                    for val in item['action_values']:
                        revenue_map[val['action_type']] = float(val['value'])

                # 2. Iterate through Actions (Counts)
                if 'actions' in item:
                    for action in item['actions']:
                        act_type = action['action_type']
                        act_count = float(action['value'])
                        act_revenue = revenue_map.get(act_type, 0.0)
                        ...
                        row = {
                            ...
                            "conversion_action": act_type,  # e.g., 'purchase', 'link_click'
                            "conversions": act_count,
                            "conversion_value": act_revenue,
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{META_CONV_ACTION_TABLE_NAME}"
    try:
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_id: str, bq_client):
    ...
    job_config = bigquery.LoadJobConfig(
        ...
        schema = [
            ...
            # Conversion Breakdown Fields
            bigquery.SchemaField("conversion_action", "STRING"),
        ...
        clustering_fields = ["account_id", "campaign_id", "conversion_action"]
    )
...
def main():
    ...
    print("--- Starting Meta Ads Conversion Action Extraction ---")
    ...
    for acc in accounts:
        ...
        try:
            df = extract_conversion_action_data(acc_id, acc_name, start_date, end_date, currency)
            if not df.empty:
