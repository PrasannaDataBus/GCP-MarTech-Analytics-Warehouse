...
META_AGE_GENDER_CONVERSIONS_TABLE = os.getenv("META_AGE_GENDER_CONVERSIONS_TABLE")
...
def extract_age_gender_conversions_data(account_id: str, account_name: str, start_date: str, end_date: str, currency: str):
    """Extracts Age, Gender Conversions data for a specific Meta Ad Account using date chunking."""
    ...
    for chunk_start, chunk_end in get_date_chunks(start_date, end_date, chunk_size = 30):
        ...
        try:
            ...
            for item in insights:
                ...
                status = campaign_status_map.get(camp_id, 'Unknown')
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{META_AGE_GENDER_CONVERSIONS_TABLE}"
    try:
...
def main():
    ...
    for acc in accounts:
        ...
        try:
            df = extract_age_gender_conversions_data(acc_id, acc_name, start_date, end_date, currency)
            if not df.empty:
