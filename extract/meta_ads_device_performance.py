# Process: Extract Device Breakdown Data and Injest into a raw schema inside a raw table
...
# Reliability Logic: Implemented 30-day Date Chunking to prevent API Timeouts
...
META_DEVICE_PERFORMANCE_TABLE_NAME = os.getenv("META_DEVICE_PERFORMANCE_TABLE_NAME")

# --- AUTHENTICATION ---
bq_client = bigquery.Client()

# --- FIELDS TO EXTRACT ---
# Note: 'impression_device' comes from 'breakdowns' param, not FIELDS
FIELDS = [
    'date_start',
    'date_stop',
    'account_id',
...
# --- EXTRACTION FUNCTION (Updated for Device) ---
def extract_device_performance_data(account_id: str, account_name: str, start_date: str, end_date: str, currency: str):
    """Extracts Device data ('impression_device') for a specific Meta Ad Account."""
    ...
    for chunk_start, chunk_end in get_date_chunks(start_date, end_date, chunk_size = 30):
        ...
        params = {
            ...
            'time_increment': 1,
            'level': 'ad',
            'breakdowns': ['impression_device'],  # <--- THE KEY DIFFERENCE
            'limit': 1000,
        ...
        try:
            ...
            for item in result_cursor:
                ...
                row = {
                    ...
                    # --- NEW BREAKDOWN FIELD ---
                    "device": item.get('impression_device', 'Unknown'),  # e.g. android_smartphone, desktop, ipad
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{META_DEVICE_PERFORMANCE_TABLE_NAME}"
    try:
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_id: str, bq_client):
    ...
    # Idempotency: Delete overlap before insert
    delete_query = f"""
    ...
    job_config = bigquery.LoadJobConfig(
        ...
        schema = [
            ...
            bigquery.SchemaField("device", "STRING"),  # Replaces publisher/position
        ...
        # Cluster by device to optimize the specific queries you need later
        clustering_fields = ["account_id", "campaign_id", "device"]
    )
...
def main():
    ...
    print("--- Starting Meta Ads 'Device' Breakdown Extraction ---")
    ...
    # ==============================================================================
    # 1. HISTORICAL BACKFILL (Run this FIRST, then comment out)
    ...
    #
    #         try:
    #             df = extract_device_performance_data(acc_id, acc_name, start_date, end_date, currency)
    #
    ...
    print("--- Meta Ads Device Load Complete ---")
