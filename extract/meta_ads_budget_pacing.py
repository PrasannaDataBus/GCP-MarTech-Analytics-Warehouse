...
META_PACING_TABLE_NAME = os.getenv("META_PACING_TABLE_NAME")

# --- AUTHENTICATION ---
bq_client = bigquery.Client()

# --- FIELDS TO EXTRACT ---
# 1. Config Fields (AdSet Object - Fetched Sync)
ADSET_FIELDS = [
    'id',
    'name',
    'daily_budget',   # In Cents/Pence
    'lifetime_budget', # In Cents/Pence
    'billing_event',
    'status'
]

# 2. Performance Fields (Insights Object - Fetched Async)
INSIGHT_FIELDS = [
    'date_start',
    'account_id',
    'account_name',
...
# --- HELPER: Fetch Budget Map (Synchronous Snapshot) ---
def get_adset_budget_map(account_id):
    """
    Fetches the current Budget Settings for all AdSets.
    This remains Synchronous because it is a configuration snapshot, not a report.
    Returns: Dict { adset_id: { daily_budget: 50.0, lifetime_budget: 0.0 } }
    """
    ...
    for attempt in range(max_retries):
        try:
            # Fetch AdSets (Configuration)
            # Limit 1000 ensures we get most adsets in one go
            adsets = account.get_ad_sets(fields = ADSET_FIELDS, params = {'limit': 1000})

            for adset in adsets:
                d_budget = float(adset.get('daily_budget', 0)) / 100.0
                l_budget = float(adset.get('lifetime_budget', 0)) / 100.0

                budget_map[adset['id']] = {
                    'daily_budget': d_budget,
                    'lifetime_budget': l_budget,
            ...
            is_rate_limit = any(
                x in error_msg for x in ['limit reached', 'too many requests', 'throughput']) or e.api_error_code() in [
                                4, 17, 613, 80004]

            if is_rate_limit:
                if attempt < max_retries - 1:
                    wait_seconds = (attempt + 1) * 60
                    print(f" Budget Config Rate Limit. Sleeping {wait_seconds}s...")
                    time.sleep(wait_seconds)
                    continue
                else:
                    print(f" Failed to fetch Budget Config after retries: {error_msg}")
            else:
                print(f"  Warning: Could not fetch budget config: {e}")
                break
        except Exception as e:
            print(f"  Warning: Unexpected error fetching budgets: {e}")
            break
...
def extract_budget_pacing_async(account_id: str, account_name: str, start_date: str, end_date: str, currency: str):
    """Extracts Spend using ASYNC Job and Merges with Budget Caps."""

    # 1. INITIALIZATION (Crucial Step: Get the Budget Config)
    print(f"  -> Fetching AdSet Budget Configuration...")
    budget_map = get_adset_budget_map(account_id)
    ...
    for chunk_start, chunk_end in get_date_chunks(start_date, end_date, chunk_size = 30):
        ...
        params = {
            ...
            'time_increment': 1,
            'level': 'adset', # Granularity: AdSet (Where budgets live)
            'limit': 1000,
        ...
        try:
            ...
            job = account.get_insights_async(fields = INSIGHT_FIELDS, params = params)
            ...
            for item in result_cursor:
                ...
                row = {
                    ...
                    "lifetime_budget_cap": config.get('lifetime_budget', 0.0),
                    "adset_status": config.get('status', 'UNKNOWN'),
...
def get_last_loaded_date(bq_client):
    table_id = f"{PROJECT_ID}.{RAW_DATASET_NAME}.{META_PACING_TABLE_NAME}"
    try:
...
def load_to_bigquery(df: pd.DataFrame, start_date: str, end_date: str, account_id: str, bq_client):
    ...
    job_config = bigquery.LoadJobConfig(
        ...
        schema = [
            ...
            bigquery.SchemaField("lifetime_budget_cap", "FLOAT"),
...
def main():
    ...
    print("--- Starting Meta Ads Budget Pacing Extraction ---")
    ...
    for acc in accounts:
        ...
        try:
            df = extract_budget_pacing_async(acc_id, acc_name, start_date, end_date, currency)
            if not df.empty:
